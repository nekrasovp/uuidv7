...
| Explicit text/batch output may remove unnecessary conversions | Version-bound microbenchmarks; the scoped PostgreSQL study includes UUID adaptation and persistence | Representative application evaluation; separate text/native/buffer boundary measurements |
...
| Integration behavior | Reproducible create/serialize/validate/store/read/retry checks for at least two intended application boundaries, with exact dependency versions; documented native-object adapter limits | Framework/PostgreSQL 17 suite passed at `f0b81a0`; combined 1.0 candidate and application-specific retry paths remain to be accepted |
...
| Honest performance claims | Exact-SHA reports for matched output types and generation guarantees; end-to-end time and memory where claimed; all skips/failures visible; no regression claim without comparable measurements | Published 0.4.0 microbenchmarks plus the scoped PostgreSQL study at `032adad`; no final 0.5.0/1.0 verdict |
...
The [incorporated PostgreSQL study](../articles/application-side-uuidv7.md#a-separate-postgresql-study-from-this-release-cycle)
is a maintainer-run experiment with a verified raw report, not independent
operator feedback. Its 29.2% historical scalar generation share motivates a
batch experiment. It does not validate demand or predict the outcome of the
unmeasured historical-batch implementation.

The [integration suite at `f0b81a0`](https://github.com/nekrasovp/uuidv7/blob/f0b81a046f0c4246d6ba04651d0eede279c1f337/docs/integration-testing.md)
and its [successful run](https://github.com/nekrasovp/uuidv7/actions/runs/35280759469)
provide executed framework/database evidence distinct from the two external
repositories. The accepted
[historical batch contract at `aefda26`](https://github.com/nekrasovp/uuidv7/blob/aefda263cfb81f0439e0f0f6df4be0d6d1165ce8/docs/api.md#historical-batches)
adds an API for evaluation; neither fact establishes operator demand or final
release performance.
